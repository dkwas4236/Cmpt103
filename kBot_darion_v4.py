...
# Program:  kBot_darion_v4.py
# -------------------------------------------
# Purpose:  This program will create a
# knock-knock joke kbot

# personality edition: Lonely Bot
...
# database for lonelyBot:

lonelyBotSayings = [("Ok but after one more joke."), ("PLEASE dont leave :-("), ("Wait wait not yet!"), ("If you love me you won't go"),
("You are my only friend, stay")]
...
# this is for when the bot says knock knock and waits for the response "whosthere"
    ...
    while cleaned_response != ("whosthere") and cleaned_response != ("kbotshutdown"):
        if cleaned_response == ("end") or cleaned_response == ("quit") or cleaned_response == ("stop") or cleaned_response == ("exit"):
            lonelyBot()
            answer = str(input(""))
            cleaned_response = clean_answer(answer).lower()
            count += 1
        elif count == 3 or (count % 3 == 0 and count != 0):
            kbotresponse = 0
...
# purpose: def lonelyBot() will overide hints and the unhelpful bot when the user types words such as "exit, end, stop, quit" and 
#          then the bot will respond with a randomly selected lonely phrase while still keeping the count. 
# parameter: none
# return: none

def lonelyBot():
    lonelysaying = random.choice(lonelyBotSayings)
    print(lonelysaying)
    print("Knock Knock.")
